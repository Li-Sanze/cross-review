...
> **Phase 4a scope 确认 (2026-04-28 修订)**：advisory only（SKILL.md + skill.yaml）。默认 host-integrated 路径为 `pack -> render-prompt -> 宿主隔离审查 -> ingest --format human`；`verify --diff --format human` 仅作为 standalone fallback，需 reviewer config / API key。无 bridge.py / pipeline_hooks。Phase 4a scope 三份总纲按此口径对齐。
