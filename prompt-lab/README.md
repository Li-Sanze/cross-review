...
Prompt Lab currently has two prompt seams:

- `python run.py --render-only ...` renders `prompt-lab/prompt-template.md` for manual paste experiments.
- `python run.py --api-only ...` calls the product verification pipeline, which renders `crossreview/core/prompt.py`; the resulting `ReviewResult.reviewer` records `prompt_source` and `prompt_version`.

Do not mix adjudications across these seams unless the summary explicitly labels the prompt source/version. Historical Round 3 outputs were marked `template: v0.3`, but that exact template was not checked into the repository, so they are not a reproducible product baseline.
